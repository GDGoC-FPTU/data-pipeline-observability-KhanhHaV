...
def validate(data):
    ...
    for record in data:
        ...
        if price_value <= 0 or category is None or str(category).strip() == '':
            error_count += 1
            continue

        cleaned_record = dict(record)
        cleaned_record['price'] = price_value
        valid_records.append(cleaned_record)

    print(f"Validation complete. {len(valid_records)} valid records kept, {error_count} invalid records dropped.")
